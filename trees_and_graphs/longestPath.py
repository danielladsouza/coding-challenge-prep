...
def longestPath(fileSystem):
    """
        The file System string consists of a compbination of paths to files
        The \t indicates the level at which each directory is at

        We can use a bottom up approach to calculate the path to a file
        We will track the depth at each level as we descend to file (leaf node)

        Bottom - up approach
    """
    path_len = defaultdict(int)
    path_len[0] = 0  # Only directories store values here

    max_len = 0

    # 1 split into individual file/ folders
    # Store the values at 1 based keys
    # user is stored at 1
    # The path to the parent is at level
    for file in fileSystem.split('\f'):
        level = file.count('\t')
        # Keep track of the length of name.
        name = len(file) - level

        # Terminating condition
        if ('.' in file):
            max_len = max(max_len, path_len[level] + name)
        else:
            # Builds upon the previous path length
            path_len[level + 1] = name + path_len[level] + 1  # for the '/'
