...
def groupingDishes(dishes):
    # Iterate over the dishes, forming a hashmap where the key is the name of each ingredient
    # This will enable you to group the names of the dishes, by ingredient
    # The next step is to get a list of the keys, sort the same
    # Now iterate through the sorted list of keys (Ingredients)
    # get the values, sort them and add them to the resulting list

    map_ingredients = defaultdict(list)

    for dish in dishes:
        for ingredient in dish[1:]:
            map_ingredients[ingredient].append(dish[0])

    sorted_ingredient_list = list(map_ingredients.keys())
    sorted_ingredient_list.sort()

    result = []

    for ingredient in sorted_ingredient_list:
        dishes = map_ingredients[ingredient]
        if len(dishes) < 2:
            continue
        dishes.sort()
        result.append([ingredient] + dishes)
